...
```{r}
tracePlot(fit.2pl.Eu, facet = FALSE, legend = TRUE) + scale_color_brewer(palette = "Set3")
```
...
```{r}
# Filter "d_ws" data-set to "nouns" only for each dialect
Ar_nouns <- filter(items_filtered, language == "Spanish (Argentinian)") %>% 
  filter(lexical_category == "nouns")

Eu_nouns <- filter(items_filtered, language == "Spanish (European)") %>% 
  filter(lexical_category == "nouns")

Mx_nouns <- filter(items_filtered, language == "Spanish (Mexican)") %>% 
  filter(lexical_category == "nouns")

Pr_nouns <- filter(items_filtered, language == "Spanish (Peruvian)") %>% 
  filter(lexical_category == "nouns")



# Join by unilemma
Ar_Eu_nouns <- Ar_nouns %>% 
  left_join(Eu_nouns, by = "uni_lemma") %>%  
  dplyr::select(uni_lemma, language.x, item_definition.x, language.y, item_definition.y) %>% 
  na.omit() 

uni_lemma_aoa_master_nouns <- uni_lemma_aoa_master %>% 
  left_join(Ar_Eu_nouns) 

diff_Ar_Eu_nouns <- uni_lemma_aoa_master_nouns %>% 
  dplyr::select(uni_lemma, language.x, item_definition.x, `Spanish (Argentinian).x`, language.y, item_definition.y, `Spanish (European).x`) %>% 
  na.omit()

```
Plot
```{r}
itemInfoPlot(fit.2pl.Ar, facet = FALSE, legend = FALSE) + scale_color_manual(values=rep("black", 1000))
```
