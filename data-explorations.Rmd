...
cleaned_uni_lemma_wider <- uni_lemma_aoa %>%
  ...
  pivot_wider(names_from = language, values_from = aoa)
#Create a data frame containing identical surface forms for European and Mexican Spanish
European_Mexican_Same <- cleaned_uni_lemma_wider %>%
  dplyr::select("uni_lemma", "cleaned_item", "Spanish (European)", "Spanish (Mexican)") %>%
  na.omit()
#Calculate the correlation between the AoA values of identical surface forms in the two dialects
correlation_European_Mexican_same <- cor(European_Mexican_Same$`Spanish (European)`, European_Mexican_Same$`Spanish (Mexican)`)
print(correlation_European_Mexican_same)
#Create a data frame containing non-identical surface forms for European and Mexican Spanish
European_Mexican_Different <- cleaned_uni_lemma_wider %>%
  dplyr::select("uni_lemma", "cleaned_item", "Spanish (European)", "Spanish (Mexican)") %>%
  filter(is.na(cleaned_uni_lemma_wider$`Spanish (European)`)| is.na(cleaned_uni_lemma_wider$`Spanish (Mexican)`))
#Write this data frame to a CSV file, clean to remove NA values and match up AoA values with only one uni_lemma. Read back in
European_Mexican_Different <- read.csv("E_M_DD.csv")
#Calculate the correlation between the AoA values of non-identical surface forms in the two dialects
correlation_European_Mexican_different <- cor(European_Mexican_Different$Spanish..European., European_Mexican_Different$Spanish..Mexican.)
print(correlation_European_Mexican_different)
```
